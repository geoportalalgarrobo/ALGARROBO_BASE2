...
def get_licitaciones(current_user_id):
    """
    Lista licitaciones con paginación opcional.
    Query params:
    ...
    try:
        ...
        base_query = """
            SELECT l.*, p.nombre as proyecto_nombre,
                   (SELECT count(*) FROM licitacion_workflow
                    WHERE licitacion_id = l.id AND estado = 'Completado') as pasos_completados
            FROM licitaciones l
        ...
        with get_db() as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                if page is not None:
                    cur.execute(
                        "SELECT COUNT(*) FROM licitaciones l JOIN proyectos p ON p.id = l.proyecto_id"
                    )
    ...
    try:
        ...
        with get_db() as conn:
            with conn.cursor() as cur:
                ...
                monto = data.get("monto_estimado") or None

                cur.execute("""
                    INSERT INTO licitaciones (proyecto_id, nombre_licitacion, id_mercado_publico,
                                              monto_estimado, usuario_creador, estado_actual_paso, estado_licitacion)
                    VALUES (%s, %s, %s, %s, %s, 1, 'Abierta') RETURNING id
                """, (data["proyecto_id"], data["nombre_licitacion"],
                      data.get("id_mercado_publico"), monto, current_user_id))
...
def update_licitacion_workflow(current_user_id, wid):
    try:
        ...
        vals.extend([current_user_id, wid])
        ...
        with get_db() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute("""
                    UPDATE licitaciones SET fecha_actualizacion = NOW()
                    WHERE id = (SELECT licitacion_id FROM licitacion_workflow WHERE id = %s)
                """, (wid,))
            conn.commit()
...
@licitaciones_bp.route("/licitaciones/docs/<filename>")
@session_required
...
@licitaciones_bp.route("/licitaciones/lib/<filename>")
@session_required
